...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/03/11 18:15:39 by adstuder          #+#    #+#             */
/*   Updated: 2019/03/11 19:07:21 by adstuder         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int		sub_llx2(int *tab, int count, char *res, int i)
{
	if (tab[4] == 1)
		tab[7] = tab[7] - 2;
...
int		printf_llx2(unsigned long long int n, const char *format)
{
	...
	count = sub_llx2(tab, count, res, i);
	return (count_free_x(count, tab, res));
}
